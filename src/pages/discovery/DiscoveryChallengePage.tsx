...
const CHALLENGES = [
  {
    ...
    editor: "Kai",
    lastEdit: "04-19 10:20",
    actions: ["Details", "Edit", "Close", "Duplicate"],
  },
  {
    id: "CH-1002",
    ...
    genres: ["Hip-Hop", "Trap"],
    editor: "Jay",
    lastEdit: "04-19 09:40",
    actions: ["Details", "Edit", "Close", "Duplicate"],
  },
  {
    id: "CH-1003",
    name: "Spring Vocal Pick",
    period: "04.17 ~ 05.01",
    status: "live",
    tracks: 19,
    votes: "1,247",
    ...
    genres: ["Ballad", "Pop"],
    editor: "Tony",
    lastEdit: "04-18 18:15",
    actions: ["Details", "Edit", "Close", "Duplicate"],
  },
  {
    id: "CH-1004",
    name: "Indie Wave Launch",
    period: "04.22 ~ 05.06",
    status: "upcoming",
    tracks: 0,
    votes: "0",
    ...
    editor: "Kai",
    lastEdit: "04-19 08:55",
    actions: ["Details", "Edit", "Start", "Duplicate"],
  },
  {
    id: "CH-1005",
    name: "Global Dance Spark",
    period: "04.25 ~ 05.09",
    status: "upcoming",
    tracks: 0,
    votes: "0",
    ...
    genres: ["EDM", "Dance"],
    editor: "Jay",
    lastEdit: "04-18 22:10",
    actions: ["Details", "Edit", "Start", "Duplicate"],
  },
  {
    id: "CH-1006",
    name: "Midnight R&B Session",
    period: "03.20 ~ 04.03",
    status: "ended",
    tracks: 54,
    votes: "4,872",
    ...
    genres: ["R&B", "Soul"],
    editor: "Kai",
    lastEdit: "04-04 11:00",
    actions: ["Details", "ResultView", "Duplicate"],
  },
  {
    id: "CH-1007",
    name: "K-Indie First Stage",
    period: "03.15 ~ 03.29",
    status: "ended",
    tracks: 38,
    votes: "2,642",
    ...
    genres: ["Indie", "Folk"],
    editor: "Tony",
    lastEdit: "03-30 15:30",
    actions: ["Details", "ResultView", "Duplicate"],
  },
];

const statusVariantMap: Record<string, "live" | "upcoming" | "ended"> = {
  live: "live",
  upcoming: "upcoming",
  ended: "ended",
};
    ...
    <div>
      <div className="grid grid-cols-6 gap-4 mb-6">
        ...
        <KpiCard
          ...
          label="Close"
          value="7"
          sub="Completed Challenge"
          valueColor="text-gray-400"
        />
        <KpiCard
          color="blue"
          label="Total Submitted Tracks"
          value="284"
          sub="All Cumulative"
        />
        <KpiCard
      ...
      <div
        className="rounded-[14px] p-6 mb-6 grid gap-6 items-center text-white border border-[#2D3B50]"
        style={{
          background:
            "linear-gradient(135deg, #1A2332 0%, #243044 60%, rgba(0,201,167,0.15) 100%)",
      ...
      >
        <div>
          ...
          <div className="text-sm text-gray-400 mb-4">
            ...
            <span className="inline-block px-2.5 py-0.5 rounded-[5px] bg-[#00C9A7] text-white font-bold text-xs ml-2">
              D-5
            </span>
          </div>
          <div className="grid grid-cols-4 gap-5">
            {[
              ...
              { label: "Use VAYLA", value: "470", mint: false },
            ].map((s) => (
              <div key={s.label}>
                ...
                <div
                  className={`text-[22px] font-extrabold ${s.mint ? "text-[#00C9A7]" : "text-white"}`}
                >
                  {s.value}
                </div>
              </div>
        ...
        <div className="flex flex-col gap-2.5 items-end">
          ...
          <button
            ...
            className="px-4 py-2 rounded-lg bg-[#00C9A7] text-white text-sm font-semibold hover:bg-[#00A88A]"
          >
            Details View →
          </button>
          <button
          ...
          <button className="px-4 py-2 rounded-lg bg-red-500 text-white text-sm font-semibold hover:bg-red-600">
            Close
          </button>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-[14px] overflow-hidden mb-6">
        <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-3.5 py-[7px] rounded-lg bg-[#00C9A7] text-white text-sm font-semibold hover:bg-[#00A88A]"
            >
              + 새 Create Challenge
            </button>
          </div>
        ...
        <div className="flex items-center gap-2.5 px-6 py-4 border-b border-gray-100 flex-wrap">
          ...
          <input
            type="text"
            placeholder="🔍 Challenge Name, Genre, ID Search..."
            ...
            <tbody>
              {CHALLENGES.map((row) => (
                <tr key={row.id} className="hover:bg-gray-50 cursor-pointer">
                  ...
                  <td
                    className={`px-3.5 py-3.5 border-t border-gray-100 font-bold ${row.status === "ended" ? "text-gray-500" : "text-[#1A2332]"}`}
                  >
                    {row.name}
                  </td>
                  <td className="px-3.5 py-3.5 border-t border-gray-100 text-xs text-gray-500 whitespace-nowrap">
                    {row.period}
                  </td>
                  ...
                  <td
                    className={`px-3.5 py-3.5 border-t border-gray-100 font-bold ${row.status === "upcoming" ? "text-gray-400" : "text-[#1A2332]"}`}
                  >
                    {row.tracks}
                  </td>
                  <td
                    className={`px-3.5 py-3.5 border-t border-gray-100 font-bold ${row.status === "upcoming" ? "text-gray-400" : "text-[#00A88A]"}`}
                  >
                    {row.votes}
                  </td>
                  <td
                    className={`px-3.5 py-3.5 border-t border-gray-100 font-semibold ${row.status === "upcoming" ? "text-gray-400" : ""}`}
                  >
                    {row.participants}
                  </td>
                  <td
                    className={`px-3.5 py-3.5 border-t border-gray-100 font-semibold ${row.status === "upcoming" ? "text-gray-400" : "text-purple-500"}`}
                  >
                    {row.vayla}
                  </td>
                  <td className="px-3.5 py-3.5 border-t border-gray-100">
                    <div className="flex flex-wrap gap-1">
                      {row.genres.map((g) => (
                        <span
                          key={g}
                          className="inline-block px-2 py-0.5 rounded text-[11px] font-semibold bg-gray-100 text-gray-600"
                        >
                          {g}
                        </span>
                      ))}
                  ...
                  <td
                    className="px-3.5 py-3.5 border-t border-gray-100"
                  ...
                  >
                    <div className="flex gap-1 flex-nowrap">
                      {row.actions.includes("Details") && (
                        <button
                          onClick={() =>
                            onNavigate?.("discovery-challenge-detail")
                          }
                          className="px-2.5 py-1 rounded-md text-xs font-semibold border border-gray-200 bg-white text-gray-600 hover:bg-gray-50"
                        >
                          Details
                        </button>
                      )}
                      ...
                      {row.actions.includes("ResultView") && (
                        <button className="px-2.5 py-1 rounded-md text-xs font-semibold bg-blue-500 text-white hover:bg-blue-600">
                          ResultView
                        </button>
                      )}
                      <button className="px-2.5 py-1 rounded-md text-xs font-semibold border border-gray-200 bg-white text-gray-600 hover:bg-gray-50">
